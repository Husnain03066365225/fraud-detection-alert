...
<style>
    ...
    .stButton>button {width: 100%; height: 55px; background: linear-gradient(135deg, #1E40AF, #3B82F6); color: white; font-size: 18px; font-weight: bold; border-radius: 12px;}
...
st.markdown("<h3 style='text-align: center; color: #1E40AF;'>AI-Powered Credit Card Fraud Detection System</h3>", unsafe_allow_html=True)
        ...
        if st.button("🔄 Continuous Retraining"):
            if st.session_state.model is not None:
                with st.spinner("Retraining with new data..."):
                    ...
                    st.info("In production, this would happen automatically using Kafka streams.")
    ...
    if st.session_state.model is None:
        ...
        if st.button("🔍 Predict + Explain with SHAP", type="primary"):
            ...
            with st.spinner("Calculating SHAP values..."):
                ...
                st.info("**Interpretation:** Longer bars = stronger influence on the prediction.")
...
• Advanced SHAP/LIME explainability for every transaction
