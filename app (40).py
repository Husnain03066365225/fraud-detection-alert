...
st.sidebar.info("""
Fraud is one of the biggest financial losses for banks. 
This system learns patterns from past transactions and predicts fraud in new ones.
It also explains **why** it flagged a transaction using SHAP.
""")
...
# TAB 1: Overview
with tab1:
    st.write("### What is this project?")
    st.write("This app demonstrates how banks can use Big Data and Machine Learning to detect fraudulent transactions in real-time.")
...
# TAB 2: Train Model
with tab2:
    st.write("### Why do we need to train the model?")
    st.info("Even though we see past fraud cases, training helps the model **learn hidden patterns** so it can predict fraud in **new unseen transactions**.")
...
with tab3:
    if st.session_state.model is None:
        ...
        if st.button("🔍 Predict + Explain with SHAP", type="primary"):
            ...
            # SHAP Explainability
            st.subheader("🔍 SHAP Explainability - Why was this decision made?")
            with st.spinner("Calculating SHAP values..."):
                ...
                st.info("**How to read:** Longer bars = more influence on the prediction. Red = pushed toward fraud.")
...
st.info("""
**Future Enhancements in Production:**
• Real Apache Kafka for live transaction streaming
• Continuous automatic model retraining with new fraud cases
